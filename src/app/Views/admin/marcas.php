...
<head>
    ...
    <title>Gestión de Marcas - Admin</title>
...
<body>
    ...
    <aside class="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="admin-marcas" class="active"><i class="ti-tag"></i> Marcas</a></li>
        ...
        <div class="content-area">
            <div class="row">
                ...
                <div class="col-lg-8">
                    <div class="content-card">
                        <div class="card-title">
                            <span>Marcas Registradas (<?php echo count($marcas ?? []); ?>)</span>
                        </div>
                        ...
                        <div id="listaMarcas">
                            ...
                            <div class="marca-card">
                                <?php if ($marca['logo']): ?>
                                <img src="img/brand/<?php echo htmlspecialchars($marca['logo']); ?>"
                                    alt="<?php echo htmlspecialchars($marca['nombre']); ?>" class="marca-logo">
                                <?php else: ?>
                                <div class="marca-logo"
                                    style="display: flex; align-items: center; justify-content: center; color: #667eea; font-size: 24px;">
                                    <i class="ti-tag"></i>
                                </div>
                                <?php endif; ?>

                                <div class="marca-info">
                                    <div class="marca-nombre"><?php echo htmlspecialchars($marca['nombre']); ?></div>
                                    <div class="marca-descripcion">
                                        <?php echo htmlspecialchars($marca['descripcion'] ?? 'Sin descripción'); ?>
                                    </div>
                                    <div class="mt-2">
                                        <span
                                            class="badge badge-<?php echo $marca['estado'] === 'activo' ? 'success' : 'secondary'; ?>">
                                            <?php echo ucfirst($marca['estado']); ?>
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div class="marca-actions">
                                    ...
                                    <button class="btn btn-sm btn-danger"
                                        onclick="eliminarMarca(<?php echo $marca['id']; ?>)">
                                        <i class="ti-trash"></i>
                                    </button>
                                </div>
                            </div>
                ...
                <div class="col-lg-4">
                    <div class="content-card">
                        <div class="card-title">
                            <span id="formTitle">Nueva Marca</span>
                        </div>
                        ...
                        <form id="formMarca" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                <label for="descripcion">Descripción/Slogan</label>
                                <textarea class="form-control" id="descripcion" name="descripcion" rows="3"></textarea>
                            </div>

                            <div class="form-group">
                                <label>Logo</label>
                                <div class="image-upload-preview" onclick="document.getElementById('logo').click()">
                                    <img id="preview_logo" style="display:none">
                                    <span id="placeholder_logo">
                                        <i class="ti-image" style="font-size: 48px; color: #ddd;"></i>
                                    </span>
                                </div>
                            ...
                            <button type="submit" class="btn btn-primary-custom btn-block">
                                ...
                                <span id="btnText">Guardar Marca</span>
                            </button>
    ...
    function previewImage(input) {
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                document.getElementById('placeholder_logo').style.display = 'none';
            }
        ...
        fetch('admin-marcas-api', {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Error: ' + (data.message || 'No se pudo guardar la marca'));
                }
            })
            .catch(error => {
    ...
    function editarMarca(marca) {
        ...
        document.getElementById('btnText').textContent = 'Actualizar Marca';
        ...
        if (marca.logo) {
            document.getElementById('preview_logo').src = 'img/brand/' + marca.logo;
            document.getElementById('preview_logo').style.display = 'block';
            document.getElementById('placeholder_logo').style.display = 'none';
        }

        // Scroll al formulario
        document.getElementById('formMarca').scrollIntoView({
            behavior: 'smooth'
        });
    }
    ...
    function eliminarMarca(id) {
        ...
        fetch('admin-marcas-api?action=eliminar&id=' + id, {
                method: 'DELETE'
            })
            .then(response => response.json())
            .then(data => {
    ...
    function limpiarFormulario() {
        document.getElementById('formTitle').textContent = 'Nueva Marca';
        document.getElementById('btnText').textContent = 'Guardar Marca';
        document.getElementById('btnCancelar').style.display = 'none';

        document.getElementById('formMarca').reset();
        document.getElementById('marca_id').value = '';
        document.getElementById('preview_logo').style.display = 'none';
        document.getElementById('placeholder_logo').style.display = 'block';
    }
    </script>
</body>
